...
# Tên model có thể đổi qua .env — nếu có GROQ_API_KEY thì mặc định dùng model Groq.
GROQ_API_BASE_URL = os.getenv("OPENAI_BASE_URL", "https://api.groq.com/openai/v1")
...
def _get_client():
    ...
    if os.getenv("GROQ_API_KEY"):
        return OpenAI(api_key=api_key, base_url=os.getenv("OPENAI_BASE_URL", GROQ_API_BASE_URL))
    return OpenAI(api_key=api_key)


def _default_model() -> str:
    _load_env()
    if os.getenv("LAB_MODEL"):
        return os.getenv("LAB_MODEL")
    if os.getenv("GROQ_API_KEY"):
        return os.getenv("GROQ_MODEL", DEFAULT_GROQ_MODEL)
    return "gpt-4o"


OPENAI_MODEL = _default_model()
...
def call_openai(
    ...
    latency = max(float(time.time() - start), 1e-6)
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        messages = [{"role": "system", "content": "Bạn là trợ lý hữu ích."}] + history
...
def run_assistant(
    ...
    while True:
        ...
        if str(user_msg).strip().lower() in {"quit", "exit"}:
            break
        ...
        num_turns += 1
        total_tokens += count_tokens(str(user_msg), OPENAI_MODEL)
        total_tokens += count_tokens(reply_text, OPENAI_MODEL)

        cost_info = estimate_cost(str(user_msg), reply_text, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def _shorten(text: Any, limit: int = 40) -> str:
        ...
        return value if len(value) <= limit else value[:37] + "..."
    ...
    for result in results:
        rows.append(
            [
                _shorten(result.get("prompt", "")),
                _shorten(result.get("gpt4o_response", "")),
                _shorten(result.get("mini_response", "")),
                f"{result.get('gpt4o_latency', 0):.4f}",
                f"{result.get('mini_latency', 0):.4f}",
            ]
        )
